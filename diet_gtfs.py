...
def process_trips(trip_ids):
    ...
    with zipfile.ZipFile('gtfs-nl.zip', 'r') as zipped:
        with zipped.open('trips.txt', 'r') as zipped_file:
            f = io.TextIOWrapper(zipped_file)
...
def clean_shapes(shape_ids):
    with zipfile.ZipFile('gtfs-nl.zip', 'r') as zipped:
        with zipped.open('shapes.txt', 'r') as zipped_file:
            f = io.TextIOWrapper(zipped_file)
            reader = csv.reader(f)
            filtered_rows = []
            filtered_rows.append(next(reader))
...
def process_stop_times(stop_ids):
    ...
    with zipfile.ZipFile('gtfs-nl.zip', 'r') as zipped:
        with zipped.open('stop_times.txt', 'r') as zipped_file:
            f = io.TextIOWrapper(zipped_file)
            reader = csv.reader(f)
            filtered_rows = []
            filtered_rows.append(next(reader))
...
def process_stops(min_lat, max_lat, min_lon, max_lon):
    ...
    with zipfile.ZipFile('gtfs-nl.zip', 'r') as zipped:
        with zipped.open('stops.txt', 'r') as zipped_file:
            f = io.TextIOWrapper(zipped_file)
            reader = csv.reader(f)
            filtered_rows = []
            filtered_rows.append(next(reader))
...
def process_routes(route_ids):
    ...
    with zipfile.ZipFile('gtfs-nl.zip', 'r') as zipped:
        with zipped.open('routes.txt', 'r') as zipped_file:
            f = io.TextIOWrapper(zipped_file)
            reader = csv.reader(f)
            filtered_rows = []
            filtered_rows.append(next(reader))
...
def clean_transfers(stop_ids, trip_ids):
    with zipfile.ZipFile('gtfs-nl.zip', 'r') as zipped:
        with zipped.open('transfers.txt', 'r') as zipped_file:
            f = io.TextIOWrapper(zipped_file)
            reader = csv.reader(f)
            filtered_rows = []
            filtered_rows.append(next(reader))
...
def clean_calendar(service_ids):
    with zipfile.ZipFile('gtfs-nl.zip', 'r') as zipped:
        with zipped.open('calendar_dates.txt', 'r') as zipped_file:
            f = io.TextIOWrapper(zipped_file)
            reader = csv.reader(f)
            filtered_rows = []
            filtered_rows.append(next(reader))
...
def clean_agencies(agency_ids):
    with zipfile.ZipFile('gtfs-nl.zip', 'r') as zipped:
        with zipped.open('agency.txt', 'r') as zipped_file:
            f = io.TextIOWrapper(zipped_file)
            reader = csv.reader(f)
            filtered_rows = []
            filtered_rows.append(next(reader))
